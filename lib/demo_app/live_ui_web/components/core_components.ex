defmodule LiveUIWeb.CoreComponents do
  @moduledoc """
  ...
  def flash(assigns) do
    ...
    <div
      ...
      role="alert"
      class={[
        "fixed top-2 right-4 w-80 sm:w-96 z-50 rounded-lg p-3 ring-1",
  ...
  def simple_form(assigns) do
    ...
    <.form :let={f} for={@for} as={@as} {@rest}>
      <div class={["mt-6 space-y-6", @class]}>
  ...
  attr :rest, :global,
    include: ~w(autocomplete cols disabled form list max maxlength min minlength
                pattern placeholder readonly required rows size step class)
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select
        ...
        name={@name}
        class={[
          "h-10 block w-full rounded-md border border-gray-300 bg-white shadow-sm",
          "focus:border-zinc-400 focus:ring-0 sm:text-sm sm:leading-6",
          @class
        ]}
        multiple={@multiple}
        {@rest}
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(LiveUIWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(LiveUIWeb.Gettext, "errors", msg, opts)
    end
  end
